<template>
  <div class="transfer transfer" :style="{ width, height }">
    <tree-transfer
      v-bind="$props"
...
export default {
  ...
  methods: {
    ...
    addToAims(useCallBack) {
      this.$refs['transfer-component'].addToAims(useCallBack);
    },
    /**
     * @name 清空选中节点
    ...
    clearChecked(type = 'all') {
      this.$refs['transfer-component'].clearChecked(type);
    },
    /**
     * @name 获取选中数据
     */
    getChecked() {
      return this.$refs['transfer-component'].getChecked();
    },
    /**
     * @name 设置选中数据
    ...
    setChecked(leftKeys = [], rightKeys = []) {
      this.$refs['transfer-component'].setChecked(leftKeys, rightKeys);
    },
    /**
     * @name 清除搜索条件
     * @param {String} type left左边 right右边 all全部 默认all
     */
    clearFilter(type) {
      this.$refs['transfer-component'].clearFilter(type);
    },
  },
};
</script>
