  ...
  <div class="component-transfer">
    ...
    <div class="transfer-left">
      ...
      <div class="transfer-main">
        ...
        >
          <span class="el-tree-node__custom custom-tree-node" slot-scope="{ node, data }">
            <slot name="content-left" :node="node" :data="data">
              <sy-icon v-if="data.icon" :name="data.icon" />
              <span class="el-tree-node__label left" :title="node.label">{{ node.label }}</span>
            </slot>
          </span>
        </sy-tree>
        ...
        >
          <span class="el-tree-node__custom custom-tree-node" slot-scope="{ node, data }">
            <slot name="content-right" :node="node" :data="data">
              <sy-icon v-if="data.icon" :name="data.icon" />
              <span class="el-tree-node__label right" :title="node.label">{{ node.label }}</span>
            </slot>
          </span>
        </sy-tree>
...
import { arrayToTree, flattenDeep, findParents } from './util';
...
export default {
  ...
  data() {
    return {
      source_data: this.from_data, // 左侧树数据
      target_data: this.to_data, // 右侧树数据
  ...
  computed: {
    // 左侧数据
    self_from_data() {
      // source_data 替代 from_data 内部控制左侧树数据
      ...
      if (this.checkStrictly) {
        ...
        console.log('this.from_array_clone: ', this.from_array_clone, _form_data);
      }
      return _form_data;
    },
    // 右侧数据
    self_to_data() {
      // target_data 替代 to_data 内部控制右侧树数据
      const _to_data = !this.arrayToTree
        ? this.target_data
        : arrayToTree(this.target_data, {
            id: this.node_key,
            pid: this.pid,
            children: this.selfDefaultProps.children,
          });
      if (this.checkStrictly) {
        ...
        console.log('this.to_array_clone: ', this.to_array_clone, _to_data);
      }
      return _to_data;
    },
    // 左侧菜单名
  ...
  methods: {
    ...
    addToAims(emit) {
      ...
      let keys = this.$refs['from-tree'].getCheckedKeys(this.checkStrictly);
      // 获取半选通过穿梭框的keys - 仅用于传送纯净的id数组到父组件同后台通信
      let harfKeys = this.$refs['from-tree'].getHalfCheckedKeys();
      // 选中节点数据
      let arrayCheckedNodes = this.$refs['from-tree'].getCheckedNodes(this.checkStrictly);
      // 半选中节点数据
    ...
    removeToSource() {
      // 获取选中通过穿梭框的keys - 仅用于传送纯净的id数组到父组件同后台通信
      let keys = this.$refs['to-tree'].getCheckedKeys(this.checkStrictly);
      // 获取半选通过穿梭框的keys - 仅用于传送纯净的id数组到父组件同后台通信
      let harfKeys = this.$refs['to-tree'].getHalfCheckedKeys();
      // 获取选中通过穿梭框的nodes 选中节点数据
      let arrayCheckedNodes = this.$refs['to-tree'].getCheckedNodes(this.checkStrictly);
      // 半选中节点数据
    ...
    checkStrictlyAdd(nodes, options) {
      const { id__, pid__, children__ } = options;
      // 第一步：将对面存在的节点抛弃
      let notInTargetNodes = differenceBy(nodes, this.to_array_clone, id__);
      console.log('add notInTargetNodes: ', nodes, this.to_array_clone);
      if (this.flattenRight) {
        // 设置选中节点为叶子节点平铺展示
        const target_nodes = notInTargetNodes.map((item) => Object.assign({}, item, { isLeaf: true }));
        this.target_data.push(...target_nodes);
      } else {
        // 第二步：整理选中数据，不知道子节点是都都选中，先将子节点都置空，下一步再组装避免判断每个节点的的子节点应不应该穿梭
        const new_nodes = notInTargetNodes.map((item) => Object.assign({}, item, { [children__]: [] }));
        // 第三步：组装能选中数据中能父子关联的，将子节点插入父节点后只需穿梭父节点
        const assembly_data = new_nodes.reduce((pre, item, idx, arr) => {
          const find_parent = arr.find((i) => i[id__] == item[pid__]);
          // 没找到父节点，将节点保留
          if (!find_parent) return pre.concat(item);
          // 找到父节点的，将节点推入父节点的children，不保留此节点
          find_parent[children__].push(item);
          return pre;
        }, []);
        console.log('add assembly_data: ', assembly_data);
        // 第四步：穿梭组装好的数据
        assembly_data.forEach((item) => {
    ...
    checkStrictlyRemove(nodes, options) {
      const { id__, pid__, children__ } = options;
      // 第一步：将对面存在的节点抛弃
      let notInTargetNodes = differenceBy(nodes, this.from_array_clone, id__);
      // console.log('remove notInTargetNodes: ', notInTargetNodes);
      // 第二步：整理选中数据，不知道子节点是都都选中，先将子节点都置空，下一步再组装避免判断每个节点的的子节点应不应该穿梭
      const new_nodes = notInTargetNodes.map((item) => {
        let new_node = Object.assign({}, item, {
          [children__]: [],
          __childrenLength: Array.isArray(item[children__]) ? item[children__].length : 0,
        });
        return new_node;
      });
      // 第三步：组装能选中数据中能父子关联的，将子节点插入父节点后只需穿梭父节点
      ...
      console.log('remove assembly_data: ', assembly_data);
      // 第四步：穿梭组装好的数据
    ...
    deepFindParent(node, options) {
      const { id__, pid__, root__, children__ } = options;
      if (node[pid__] === root__) {
        this.$refs['from-tree'].append(node);
        return;
      }
      if (this.from_array_clone.length === 0) return;
      const parentInThere = this.from_array_clone.some((item) => item[id__] === node[pid__]);
      console.log('parentInThere: ', parentInThere, this.from_array_clone);
      if (parentInThere) {
        this.$refs['from-tree'].append(node, node[pid__]);
      } else {
        const parentHere = this.to_array_clone.find((item) => item[id__] === node[pid__]);
        const _parent = Object.assign({}, parentHere, { [children__]: [node] });
    ...
    checkStrictlyTransfer(nodes, options, isAdd) {
      ...
      const new_nodes = nodes.map((i) => {
        let new_node = Object.assign({}, i, {
          [options.children__]: [],
          __childrenLength: Array.isArray(i[options.children__]) ? i[options.children__].length : 0,
        });
        return new_node;
      });
      // 先组合选中节点中能拼接起来的父子节点
      ...
      assembly_data.forEach((i) => {
        ...
        // 计算此节点的子节点有没有全部参与穿梭
        const children_num = Array.isArray(i[options.children__]) ? i[options.children__].length : 0;
    ...
    findParentInTarget(item, options, from_data, to_data, from_ref, to_ref, isAdd) {
      ...
      const parentTransferred = this.strictly_transferred.some((t) => t[options.id__] === item[options.pid__]);
